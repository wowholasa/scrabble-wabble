...
module State = 
    ...
    let mkState b d pn h pt= {board = b; dict = d;  playerNumber = pn; hand = h; placedTiles = pt}
...
module Scrabble =
    ...
    // let idToChar (id:uint32) (pieces:Map<uint32, Set<char * int>>) : char = 
       // fst (Set.minElement (Map.find id pieces))
    
    // let charToUint (c:char) (pieces:Map<uint32, Set<char * int>>) : uint32 = 
    ...
    let playGame cstream pieces (st : State.state) =
        ...
        let rec aux (st : State.state) =
            ...
            if st.playerNumber = 1u && Map.isEmpty st.placedTiles then
                let move = placeFirstWord st pieces
